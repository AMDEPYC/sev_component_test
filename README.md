...
```
To install the python module packaging run:
```
...
```
# Usage
Once the host is set-up, in order to run the tool simply use the command:
```
$ python3 SEVComponentTest.py
```
...
- Current OS distribution [ cat /etc/os-release ] Found: ubuntu 21.04 Expected: (comparing against known minimum version list) OK
- Kernel [ uname -r ] Found: 5.11.0-49-generic Expected: 4.16 minimum OK
...
- Libvirt version [ virsh -V ] Found: 7.0.0 Expected: 4.5 minimum OK
- LibVirt SEV enablement [ virsh domcapabilities | grep sev ] Found: <sev supported='yes'> Expected: <sev supported='yes'> OK
- QEMU version [ qemu-system-x86_64 --version ] Found: 5.2.0 Expected: 2.12 minimum OK
- OVMF package installation [ dpkg --list ] Found: 2020.11 Expected: commit 2018-07-06 OK
All of the found OVMF paths:
- OVMF Path Found: /usr/share/OVMF/OVMF_VARS.fd Commit date Found: 2020-11-01  Expected: commit 2018-07-06 OK
SEV COMPONENT SUPPORT PASS

Comparing Host OS componenets to known SEV-ES minimum versions:
- Kernel [ uname -r ] Found: 5.11.0-49-generic Expected: 5.11 minimum OK
...
- Libvirt version [ virsh -V ] Found: 7.0.0 Expected: 4.5 minimum OK
- LibVirt SEV enablement [ virsh domcapabilities | grep sev ] Found: <sev supported='yes'> Expected: <sev supported='yes'> OK
- QEMU version [ qemu-system-x86_64 --version ] Found: 5.2.0 Expected: 6.0 minimum FAIL
- OVMF package installation [ dpkg --list ] Found: 2020.11 Expected: commit 2020-11-01 OK
All of the found OVMF paths:
- OVMF Path Found: /usr/share/OVMF/OVMF_VARS.fd Commit date Found: 2020-11-01  Expected: commit 2020-11-01 OK
SEV-ES COMPONENT SUPPORT FAIL
```
...
```
$ python3 SEVComponentTest.py --stopfailure
```
or
```
$ python3 SEVComponentTest.py -s
```
...
```
$ python3 SEVComponentTest.py --nonverbose
```
or
```
$ python3 SEVComponentTest.py -nv
```
...
To use this flag, use the command:
```
$ python3 SEVComponentTest.py --test [features to be tested]
```
or
```
$ python3 SEVComponentTest.py -t [features to be tested]
```
Features (sev, sev-es or sme) written in lower-case and separated by a space.

Example feature testing for SME and SEV only:
```
$ python3 SEVComponentTest.py -t sme sev
```
...
```
$ python3 SEVComponentTest.py --testlocal "qemu-system-x86_64 -enable-kvm -cpu EPYC -machine q35 -smp 4,maxcpus=64 -m 2048M,slots=5,maxmem=30G -drive if=pflash,format=raw,unit=0,file=/usr/local/share/qemu/OVMF_CODE.fd,readonly -drive if=pflash,format=raw,unit=1,file=OVMF_VARS.fd -netdev user,id=vmnic -device e1000,netdev=vmnic,romfile= -drive file=ubuntu-18.04-server-cloudimg-amd64.img,if=none,id=disk0 -drive file=seed.iso,if=none,id=cd0 -object sev-guest,id=sev0,cbitpos=47,reduced-phys-bits=1 -machine memory-encryption=sev0 -nographic"
```
or the user can leave the command blank to launch the UI:
```
$ python3 SEVComponentTest.py -tl
...
```
$ python3 SEVComponentTest.py --autotest
```
or
```
$ python3 SEVComponentTest.py -at
```
Example of result:
```
